...
\begin{enumerate}
    \item \textbf{RQ1:} TF-IDF kết hợp Linear SVM có đủ mạnh cho phân loại sentiment review TMĐT tiếng Việt trong phạm vi dữ liệu hiện có không?
...
\node[box, below=of m1] (m2) {\textbf{Module 2: Sentiment Classification}\\TF-IDF + Logistic Regression, TF-IDF + Linear SVM, select by validation macro-F1};
\node[box, below=of m2] (m3) {\textbf{Module 3: Review Analytics}\\sentiment distribution, rating-sentiment crosstab, top negative reviews};
\node[box, below=of m3] (m4) {\textbf{Module 4: Retrieval/RAG}\\multilingual-E5, FAISS IndexFlatIP, metadata filtering, evidence-grounded answer};
...
\midrule
Module 1 & Raw review, rating, metadata & Dữ liệu sentiment đã xử lý, RAG corpus và data summary. \\
Module 2 & Train/valid/test sentiment & Model sentiment tốt nhất và bảng metric tổng hợp. \\
Module 3 & Dữ liệu sentiment/RAG đã xử lý & Phân phối sentiment, crosstab rating-sentiment, top negative reviews, figures \\
Module 4 & RAG corpus, FAISS index, metadata, query & Review liên quan, answer có citation, retrieval evaluation \\
Module 5 & Metric Module 2/4 và error files & Final summary, discussion, conclusion, manual precision summary \\
Module 6 & Corpus/index final và benchmark query & Hybrid retrieval output, benchmark reference, hướng nâng cấp \\
...
\code{artifact_manifest.json} & Danh sách artifact khi đóng gói nộp bài. \\
\bottomrule
\end{longtable}
...
Backend dùng FastAPI trong \code{backend/app.py}. Backend chịu trách nhiệm load model sentiment, load FAISS index/metadata, chạy retrieval và trả JSON cho frontend. Frontend gồm \code{frontend/index.html}, \code{frontend/styles.css} và \code{frontend/app.js}. Frontend gọi API backend bằng HTTP, sau đó render kết quả dưới dạng card, badge và panel kỹ thuật.
...
Hợp đồng dữ liệu còn giúp debug. Khi giao diện không load được backend, có thể kiểm tra \code{/api/health}. Khi RAG không trả kết quả, có thể kiểm tra \code{models/module4/review_faiss.index} và \code{review_metadata.parquet}. Khi sentiment không chạy, có thể kiểm tra \code{models/module2/best_model.joblib}. Cách chia module giúp lỗi được khoanh vùng nhanh hơn so với viết tất cả logic trong một notebook duy nhất.
