...
Thực nghiệm sentiment sử dụng dữ liệu 5.162 review sau dedup. Dữ liệu được chia train/valid/test với phân phối lớp như Chương 4. Các mô hình so sánh gồm majority baseline, TF-IDF + Logistic Regression và TF-IDF + Linear SVM. Tiêu chí chọn model là validation macro-F1, vì macro-F1 đánh giá đều các lớp và phù hợp khi lớp neutral ít hơn.
...
\begin{table}[H]\centering\caption{So sánh mô hình sentiment trên tập test}\label{tab:sentiment-model-comparison}\begin{tabular}{lrrr}\toprule Mô hình & Accuracy & Macro-F1 & Weighted-F1 \\\midrule Majority baseline & 0,4297 & 0,2004 & 0,2583 \\ TF-IDF + Logistic Regression & 0,9174 & 0,8940 & 0,9176 \\ TF-IDF + Linear SVM & \textbf{0,9329} & \textbf{0,9098} & \textbf{0,9315} \\\bottomrule\end{tabular}\end{table}

Bảng \ref{tab:sentiment-model-comparison} cho thấy hai mô hình TF-IDF đều vượt xa majority baseline. Linear SVM là mô hình tốt nhất trong các baseline đã thử. So với majority baseline, Linear SVM tăng test accuracy từ 0,4297 lên 0,9329 và macro-F1 từ 0,2004 lên 0,9098. Điều này chứng minh mô hình thật sự học được tín hiệu từ văn bản, không chỉ dựa vào lớp nhiều nhất.
...
\safeimage{../figures/module2/model_comparison_macro_f1.png}{So sánh macro-F1 giữa các mô hình sentiment.}{fig:model-macro-f1}{0.82\textwidth}
...
Manual Precision@k của retrieval baseline được tổng hợp trong \code{results/module5/manual_precision_overall.csv}. Kết quả gồm 15 query đánh giá.
...
Module 6 được đánh giá bằng benchmark nhỏ trong \code{reports/module6_retrieval_eval_reference_from_hybrid_project.csv} và hình \code{figures/module6/module6_retrieval_comparison_reference.png}. Benchmark này kiểm tra keyword hit-rate và aspect hit-rate, không thay thế đánh giá relevance thủ công lớn.
...
Sentiment classification là phần đạt kết quả mạnh nhất, với Linear SVM macro-F1 khoảng 0,9098. Retrieval/RAG đã hoạt động end-to-end và có citation, nhưng Precision@5 còn ở mức baseline khoảng 0,5067. Module 6 chỉ ra hướng cải thiện retrieval theo aspect, nhưng cần đánh giá lớn hơn trước khi khẳng định hiệu quả tổng quát.

\section{Diễn giải sâu kết quả sentiment}
Linear SVM vượt Logistic Regression khoảng 0,0158 macro-F1 trên tập test. Chênh lệch này không quá lớn nhưng nhất quán với validation macro-F1. Điều này cho thấy Linear SVM phù hợp hơn với dữ liệu TF-IDF sparse trong đề tài. Majority baseline có macro-F1 rất thấp vì chỉ dự đoán lớp positive, làm F1 của negative và neutral bằng 0.
...
Các biểu đồ accuracy và macro-F1 giúp so sánh nhanh hiệu quả giữa các mô hình sentiment. Trong đó, macro-F1 có ý nghĩa quan trọng hơn accuracy vì phản ánh hiệu quả trên cả lớp neutral, vốn là lớp ít mẫu và khó phân biệt hơn. Confusion matrix bổ sung góc nhìn chi tiết hơn bằng cách cho thấy mẫu nào bị nhầm giữa negative, neutral và positive.
