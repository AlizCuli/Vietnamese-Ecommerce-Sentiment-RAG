...
Theo \code{data/reports/data_summary.json}, dữ liệu sentiment có 5.971 dòng thô. Sau làm sạch vẫn còn 5.971 dòng, sau xử lý duplicate còn 5.162 dòng. Có 46 duplicate conflict bị loại. Conflict duplicate là trường hợp nội dung hoặc bản ghi trùng nhưng nhãn/rating mâu thuẫn, nếu giữ lại sẽ gây nhiễu cho mô hình.
...
\section{Dữ liệu RAG}
Dữ liệu RAG được lưu ở \code{data/processed/rag_corpus_module4.csv}. Theo \code{models/module4/module4_index_config.json}, bản final của RAG index có 82.677 documents. Đây là con số cần ưu tiên khi trình bày bản final, vì nó phản ánh artifact đang dùng trong demo cuối.

Trong \code{data/reports/data_summary.json} vẫn có trường \code{rag_total_documents} bằng 45.259. Đây là summary của một giai đoạn build cũ hoặc bản corpus trước khi mở rộng. Báo cáo này sử dụng con số 82.677 cho bản final, đồng thời ghi rõ sự khác biệt nếu cần đối chiếu.
\begin{table}[H]
\centering
...
Index file & \code{review_faiss.index} \\
Metadata file & \code{review_metadata.parquet} \\
\bottomrule
\end{tabularx}
\end{table}
